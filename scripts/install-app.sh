...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Orchestra-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if pgrep -x "Orchestra" >/dev/null 2>&1; then
        ...
        if pgrep -x "Orchestra" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Orchestra" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Orchestra has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Orchestra'${NC}"
    echo ""
    ...
    APPIMAGE_INSTALL_PATH="$APP_DIR/Orchestra-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Orchestra.*AppImage" >/dev/null 2>&1; then
        info "Stopping Orchestra..."
        pkill -f "Orchestra.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
APPIMAGE_PATH="$HOME/.craft-agent/app/Orchestra-x64.AppImage"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Orchestra-x64.AppImage"
