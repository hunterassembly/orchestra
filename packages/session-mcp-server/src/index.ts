...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`Orchestra Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Orchestra Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Orchestra Docs server is not connected. Tool '${name}' unavailable.`);
  }
