...
export interface ChatGptAuthTokensLoginParams {
  ...
  accessToken: string;
  /**
   * ChatGPT account/workspace identifier.
   * Required by newer Codex app-server versions.
   */
  chatgptAccountId: string;
  /**
   * Optional ChatGPT plan type (for example "pro").
   */
  chatgptPlanType?: string | null;
  /**
   * Legacy field for older app-server versions.
   * Safe to include when available.
   */
  idToken?: string;
}
...
export class AppServerClient extends EventEmitter {
   ...
   *
   * @param tokens - Access token + account metadata from ChatGPT OAuth
   */
   ...
   * @param tokens - Refreshed token payload (new + legacy-compatible fields)
   */
  ...
  private async initialize(): Promise<void> {
    ...
    // Newer Codex app-server versions gate `chatgptAuthTokens` behind the
    // experimental API capability. Older versions may not understand this field.
    const paramsWithCapabilities: InitializeParams & {
      capabilities: {
    ...
    try {
      response = await this.request<InitializeResponse>('initialize', paramsWithCapabilities);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const mayBeLegacyServer = /unknown field.*capabilities|missing field.*clientInfo|invalid params/i.test(message);
      if (!mayBeLegacyServer) {
        throw error;
      }
      this.debug(`initialize with capabilities failed (${message}), retrying legacy initialize`);
      response = await this.request<InitializeResponse>('initialize', baseParams);
    }
