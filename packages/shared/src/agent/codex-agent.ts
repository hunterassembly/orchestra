...
import { getModelById, getModelProvider, getDefaultSummarizationModel } from '../config/models.ts';
...
import {
  ...
  generateCodexConfig,
} from '../codex/config-generator.ts';
...
import {
  extractChatGptAccessTokenMetadata,
  refreshChatGptTokens,
  type ChatGptTokens,
} from '../auth/chatgpt-oauth.ts';
...
const DEFAULT_CODEX_MODEL = 'pi/gpt-5.3-codex';

type CodexChatGptAuthPayload = {
  accessToken: string;
 ...
 */
function resolveCodexModelId(modelId: string, fallback: string): string {
  if (modelId === 'codex' || modelId === 'codex-mini') {
    return fallback;
  }
  // Legacy connection model IDs were stored as `pi/<model-id>`.
  // Codex app-server now expects raw IDs (for example `gpt-5.3-codex`).
...
export class CodexAgent extends BaseAgent {
  protected override backendName = 'Codex';
  ...
  private _pendingChatGptTokens: CodexChatGptAuthPayload | null = null;
  ...
  private get codexHome(): string | undefined {
    ...
    if (typeof runtime.codexHome === 'string' && runtime.codexHome.length > 0) {
      return runtime.codexHome;
    }
    const sessionPath = this.getSessionStoragePath();
    return sessionPath ? join(sessionPath, '.codex-home') : undefined;
  }
  ...
  private async ensureClient(): Promise<AppServerClient> {
    ...
    if (codexHome) {
      ...
      this.debug(`Using custom CODEX_HOME: ${codexHome}`);
    }
  ...
  /**
   * Build the payload expected by Codex app-server `chatgptAuthTokens` login.
   *
   * Newer Codex versions require `chatgptAccountId` (and accept optional
   * `chatgptPlanType`). We derive these from access-token claims when not
   * explicitly available, with a fallback to prior stored account ID.
   */
  private resolveChatGptAuthPayload(
    tokenData: {
      accessToken: string;
      idToken?: string;
      chatgptAccountId?: string;
  ...
  ): CodexChatGptAuthPayload | null {
    const metadata = extractChatGptAccessTokenMetadata(tokenData.accessToken);
    const chatgptAccountId =
      tokenData.chatgptAccountId ??
    ...
    return {
      ...
      chatgptAccountId,
      chatgptPlanType: tokenData.chatgptPlanType ?? metadata?.chatgptPlanType ?? null,
      ...(tokenData.idToken ? { idToken: tokenData.idToken } : {}),
    };
  }
    ...
    if (this.tokenRefreshInProgress) {
      ...
      try {
        ...
        if (storedCreds?.accessToken) {
          const payload = this.resolveChatGptAuthPayload(
            {
              accessToken: storedCreds.accessToken,
              idToken: storedCreds.idToken,
              chatgptAccountId: storedCreds.chatgptAccountId,
              chatgptPlanType: storedCreds.chatgptPlanType,
            },
            params.previousAccountId
          );
          if (!payload) {
            throw new Error('Unable to resolve chatgptAccountId from refreshed token');
          }
          await this.client?.respondToTokenRefresh(params.requestId, payload);
  ...
  private async _doTokenRefresh(params: ChatGptTokenRefreshRequestParams & { requestId: RequestId }): Promise<void> {
    try {
      ...
      const payload = this.resolveChatGptAuthPayload(
        ...
        storedCreds.chatgptAccountId ?? params.previousAccountId
      );
      if (!payload) {
        throw new Error('Unable to resolve chatgptAccountId from refreshed token');
      }
      ...
      this.client?.respondToTokenRefresh(params.requestId, payload);
  ...
  async injectChatGptTokens(tokens: ChatGptTokens): Promise<void> {
    ...
    if (!payload) {
      throw new Error('ChatGPT token missing required chatgptAccountId claim');
    }
    ...
    // Inject into Codex
    await client.accountLoginWithChatGptTokens(payload);
  ...
  async tryInjectStoredChatGptTokens(): Promise<boolean> {
    try {
      ...
      if (storedCreds.expiresAt && Date.now() > storedCreds.expiresAt - 5 * 60 * 1000) {
        ...
        if (storedCreds.refreshToken) {
          ...
          const payload = this.resolveChatGptAuthPayload(
            ...
            storedCreds.chatgptAccountId
          );
          if (!payload) {
            this.debug('Unable to resolve chatgptAccountId for refreshed ChatGPT tokens');
            return false;
          }
          ...
          if (this.client?.isConnected()) {
            await this.client.accountLoginWithChatGptTokens(payload);
            this.debug('Refreshed ChatGPT tokens injected directly');
          } else {
            this._pendingChatGptTokens = payload;
      ...
      // Need accessToken to inject. chatgptAccountId is derived from claims if possible.
      if (!storedCreds.accessToken) {
      ...
      if (!payload) {
        this.debug('Unable to resolve chatgptAccountId from stored ChatGPT credentials');
        return false;
      }
  ...
  async generateTitle(prompt: string): Promise<string | null> {
    ...
    const model = resolveCodexModelId(this.config.miniModel, this.config.miniModel);
  ...
  protected override async *chatImpl(
    ...
    try {
      ...
      const model = resolveCodexModelId(
        ...
        this._model || DEFAULT_CODEX_MODEL,
      );
      if (this.codexThreadId) {
        ...
        try {
          await client.threadResume({
            ...
            model: miniConfig.enabled ? resolveCodexModelId(this.config.miniModel ?? DEFAULT_CODEX_MODEL, this._model || DEFAULT_CODEX_MODEL) : null,
            modelProvider: null,
            cwd: null,
            approvalPolicy: null,
            sandbox: null,
          ...
          this.codexThreadId = response.thread.id ?? null;
          if (!this.codexThreadId) {
            throw new Error('Codex thread start failed: missing thread ID');
          }
  ...
  private buildUserInput(
    ...
    // Skills are already processed in BaseAgent.chat() before chatImpl().
  ...
  override async applyBridgeUpdates(context: BridgeUpdateContext): Promise<void> {
    const codexHome = this.codexHome;
    if (!codexHome) {
      this.debug('Skipping bridge update: CODEX_HOME unavailable');
      return;
    }
    ...
    const bridgeConfigJson = generateBridgeConfig(context.enabledSources);
    await writeFile(bridgeConfigPath, bridgeConfigJson, 'utf8');

    const config = generateCodexConfig({
      sources: context.enabledSources,
      mcpServerConfigs: context.mcpServers,
      bridgeServerPath,
      bridgeConfigPath,
      ...
      plansFolderPath: getSessionPlansPath(context.workspaceRootPath, context.sessionId),
      nodePath,
    });

    await writeFile(codexConfigPath, config.toml, 'utf8');
    this.debug(
      `Updated Codex config (${context.context}): ${config.mcpSources.length} MCP, ${config.apiSources.length} API, warnings=${config.warnings.length}`,
    );
    for (const warning of config.warnings) {
      this.debug(`[Codex config warning] ${warning.sourceSlug}: ${warning.message}`);
    }
  ...
  /**
   * Keep base source-state tracking and log Codex source inventory.
   */
  override async setSourceServers(
    ...
    this.debug(
      `Source servers synced: ${Object.keys(mcpServers).length} MCP, ${Object.keys(apiServers).length} API`,
    );
  }
  ...
  async queryLlm(request: LLMQueryRequest): Promise<LLMQueryResult> {
    ...
    const model = resolveCodexModelId(rawModel, this.config.miniModel ?? this._model ?? DEFAULT_CODEX_MODEL);
