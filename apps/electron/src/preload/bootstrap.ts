...
if (wsMode === 'local') {
  ...
  ;(api as any).renameVaultText = (vaultRootPath: string, oldRelativePath: string, newRelativePath: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.VAULT_RENAME_TEXT, vaultRootPath, oldRelativePath, newRelativePath)

  ;(api as any).getPushToTalkWhisper = () =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_GET_PUSH_TO_TALK_WHISPER)
  ;(api as any).setPushToTalkWhisper = (enabled: boolean) =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_SET_PUSH_TO_TALK_WHISPER, enabled)
  ;(api as any).getWhisperMicrophoneId = () =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_GET_WHISPER_MICROPHONE_ID)
  ;(api as any).setWhisperMicrophoneId = (deviceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_SET_WHISPER_MICROPHONE_ID, deviceId)
  ;(api as any).startNativePushToTalk = (preferredDeviceLabel?: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_NATIVE_PTT_START, preferredDeviceLabel)
  ;(api as any).stopNativePushToTalkAndTranscribe = () =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_NATIVE_PTT_STOP_AND_TRANSCRIBE)
  ;(api as any).transcribeWithLocalWhisper = (audioBase64: string, mimeType: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.INPUT_TRANSCRIBE_LOCAL_WHISPER, audioBase64, mimeType)
}
